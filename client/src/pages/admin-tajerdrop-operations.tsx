  ...
  return <div className="min-h-[100dvh]" style={{ background: PAGE, color: NAVY }}>
    ...
    <Dialog open={!!payTarget} onOpenChange={v => !v && setPayTarget(null)}>
      <DialogContent>
        ...
        {/* Les coordonnees sont rappelees ici plutot que dans un autre ecran :
            c'est au moment de saisir le versement qu'on recopie un RIB. */}
        {payTarget?.bankRib ? (
          <div className="rounded-lg border p-3 text-sm">
            <div className="font-medium" style={{ color: NAVY }}>{payTarget.bankName || "Établissement non précisé"}</div>
            <div className="mt-0.5 font-mono text-slate-600">{String(payTarget.bankRib).replace(/(.{4})/g, "$1 ").trim()}</div>
            {payTarget.bankHolder && <div className="text-xs text-slate-400">{payTarget.bankHolder}</div>}
          </div>
        ) : (
          <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800">
            Ce seller n'a pas renseigné ses coordonnées bancaires.
          </div>
        )}
