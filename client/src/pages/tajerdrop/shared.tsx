...
export function Empty({ title, text, href, action }: { title: string; text: string; href?: string; action?: string }) { return <div className="rounded-2xl border border-dashed border-[#FF6B35]/40 bg-[#fffaf0] p-12 text-center"><PackageOpen className="mx-auto mb-3 h-9 w-9 text-[#FF6B35]"/><h3 className="font-semibold text-[#0F172A]">{title}</h3><p className="mx-auto mt-1 max-w-md text-sm text-slate-500">{text}</p>{href && <Link href={href} className="mt-5 inline-flex items-center gap-2 rounded-lg bg-[#0F172A] px-4 py-2 text-sm font-semibold text-white">{action}<ArrowRight className="h-4 w-4"/></Link>}</div>; }
export function PageHead({ eyebrow, title, text }: { eyebrow?: string; title: string; text?: string }) { return <div className="mb-6"><p className="text-[11px] font-bold uppercase tracking-[.18em] text-[#FF6B35]">{eyebrow || "TAJERDROP · SELLER"}</p><h1 className="mt-1 text-3xl font-bold tracking-tight text-[#0F172A]">{title}</h1>{text && <p className="mt-1 text-sm text-slate-500">{text}</p>}</div>; }
